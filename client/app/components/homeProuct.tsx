...
export default function HomeProduct(props: any) {
    ...
    return (
        <div className="bg-white p-4 shadow rounded max-w-screen-lg mx-auto">
            <img src={sp.hinh} alt="Product" className="w-full h-58 object-cover rounded mb-4 transition-transform duration-300 hover:scale-105"/>
            <h3 className="text-base font-bold mt-2 mb-4 hover:text-blue-500 text-center line-clamp-1">
                <Link href={`/product/detail/${sp.id}`}>{sp.ten_sp}</Link>
            </h3>
            <div className="flex items-center justify-between mb-4">
                <p className="text-base font-bold">Giá: <span className="text-red-500">{sp.gia_km.toLocaleString("vi-VN")} VNĐ</span></p>
